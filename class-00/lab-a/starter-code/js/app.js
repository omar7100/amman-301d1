...
function displayPics(){
  ...
  // we need to invoked the rando inside the function , when we changed var to const we had the error.  
  // Variables declared with the const keyword can have Block Scope.
...
function showList() {
  for(const i = 0; i < allProducts.length; i++) {
    let l
  iEl = document.createElement('li');
    liE
  l.textContent = allProducts[i].name + ' has ' + allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views';
